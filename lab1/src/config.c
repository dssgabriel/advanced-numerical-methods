...
void help(char const* bin)
{
    ...
    fprintf(stderr, "  -p, --parallel [NB_CORES]   Enable parallel mode and optionally specify "
                    "number of threads.\n");
    fprintf(stderr,
            "  -r, --repetitions <NB_REPS> Specify number of repetitions of the BLAS kernel.\n");
    fprintf(stderr,
            "  -o, --output <FILENAME>     Specify the output filename (stdout by default).\n\n");
}
...
config_t config_from(int argc, char* argv[argc + 1])
{
    ...
    for (;;) {
        ...
        switch (curr_opt) {
            ...
            default:
                fprintf(stderr, BOLD RED "error:" RESET " unrecognized option.\n"
