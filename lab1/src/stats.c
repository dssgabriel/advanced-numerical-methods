...
void stats_compute(stats_t* self)
{
    ...
    if (MAX_SAMPLES & 1) {
        ...
        self->median = (self->samples[MAX_SAMPLES >> 1] + self->samples[(MAX_SAMPLES >> 1) + 1]) / 2.0;
    }
...
int stats_dump(stats_t const* self, char const* filename)
{
    ...
    fprintf(ofp, "%s; %u; %zu; %zu; %2.3lf; %2.3lf; %2.3lf; %2.3lf; %2.3lf%%; %2.3lf; %2.3lf\n",
            self->title, self->blas_lvl, self->nb_threads, self->nb_elems,
            self->min, self->mean, self->max, self->median, self->stddevp,
            self->mem_throughput, self->ops_throughput);
